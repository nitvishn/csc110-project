...
def calculate_relation(post: RedditObject, topic: str) -> float:
    """
    Determines whether a reddit post is related to the given topic or not by scanning for
    keywords in the post title and body - 1 represents fully related, and 0 represents
...
def posts_in_interval(posts: list[RedditObject], start: datetime.datetime,
                      end: datetime.datetime) -> list[RedditObject]:
    """
...
def determine_post_topic(post: RedditObject, threshold: float = 0.5) -> list[str]:
    """
    Determines the topics to which the given post is related. Threshold provides the minimum
    degree of relation needed for topics to be related.
...
def calculate_topic_popularity(posts: list[RedditObject], topic: str) -> float:
    """
...
def get_time_array(start: datetime.date, end: datetime.date,
                   resolution: datetime.timedelta) -> list[datetime.date]:
    """
...
def new_cases_in_interval(case_data: list[tuple[datetime.datetime, int]],
                          start: datetime.datetime, end: datetime.datetime) -> int:
    """
    Returns the number of new cases in an interval of time
    """
    return sum([case[1] for case in case_data if start <= case[0] <= end])


def new_cases_at_times(case_data: list[tuple[datetime.datetime, int]],
                       times: list[datetime.datetime],
                       resolution: list[datetime.datetime]) -> list[int]:
    """
    Returns a list of the amount of new cases at certain time intervals
    """
...
if __name__ == "__main__":
    ...
    python_ta.check_all(config={
        ...
        'extra-imports': ['math', 'python_ta.contracts', 'hypothesis.strategies',
                          'flair.data', 'flair.models', 'joblib', 'datetime',
